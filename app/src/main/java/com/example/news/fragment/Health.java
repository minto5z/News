...
import com.example.news.viewmodel.HealthViewModel;
import com.example.news.viewmodel.NewsViewModelFactory;

import java.util.ArrayList;
import java.util.List;

public class Health extends Fragment implements NewsAdapter.ClickListener {
    private RecyclerView recyclerView;
    private NewsAdapter adapter;
    private HealthViewModel viewModel;
    private List<News> healthData;
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        recyclerView = view.findViewById(R.id.health_recycleView);
        progressBar = view.findViewById(R.id.health_progressBar);
        NewsRepository repository = NewsApplication.repository;
        viewModel = new ViewModelProvider(this, new NewsViewModelFactory(repository)).get(HealthViewModel.class);
        RecyclerView.LayoutManager managers = new LinearLayoutManager(getActivity());
        adapter = new NewsAdapter(this);
        recyclerView.setLayoutManager(managers);
        recyclerView.setAdapter(adapter);
        viewModel.healthLiveData.observe(getViewLifecycleOwner(), new Observer<Response<List<News>>>() {
            @Override
    ...
    public void onItemClick(int position) {
        Uri uri = Uri.parse(healthData.get(position).getUrl());
